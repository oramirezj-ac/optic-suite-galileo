...
CREATE TABLE IF NOT EXISTS `abonos` (
  ...
  KEY `fk_abonos_ventas` (`id_venta`),
  CONSTRAINT `fk_abonos_ventas` FOREIGN KEY (`id_venta`) REFERENCES `ventas` (`id_venta`) ON DELETE CASCADE
...
-- Volcando estructura para tabla optica_galileo_db.catalogo_productos_medicos
CREATE TABLE IF NOT EXISTS `catalogo_productos_medicos` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `nombre` (`nombre`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Catálogo de productos médicos vendidos en consultas';
...
CREATE TABLE IF NOT EXISTS `consultas` (
  ...
  `motivo_consulta` enum('Refractiva','Médica') NOT NULL DEFAULT 'Refractiva',
  ...
  `diagnostico_dx` varchar(255) DEFAULT NULL,
  `tratamiento_rx` text DEFAULT NULL,
  `costo_servicio` decimal(10,2) NOT NULL DEFAULT 0.00,
  `metodo_pago` varchar(50) DEFAULT NULL,
  `estado_financiero` enum('cobrado','pendiente','cortesia','garantia') NOT NULL DEFAULT 'cobrado',
  ...
  `dp_cerca` int(11) DEFAULT NULL COMMENT 'Distancia Pupilar de Cerca (DP Lejos - 2mm)',
...
-- Volcando estructura para tabla optica_galileo_db.consulta_productos_medicos
CREATE TABLE IF NOT EXISTS `consulta_productos_medicos` (
  ...
  CONSTRAINT `fk_consulta_productos_consulta` FOREIGN KEY (`consulta_id`) REFERENCES `consultas` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_consulta_productos_producto` FOREIGN KEY (`producto_id`) REFERENCES `catalogo_productos_medicos` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Productos médicos vendidos en consultas';
...
CREATE TABLE IF NOT EXISTS `graduaciones` (
  ...
  `tipo` enum('autorrefractometro','foroptor','ambulatorio','lensometro','final','externa') NOT NULL,
...
CREATE TABLE IF NOT EXISTS `ventas` (
  ...
  `consulta_id` int(11) DEFAULT NULL COMMENT 'ID de la consulta que generó esta venta',
  ...
  `vendedor_armazon` varchar(50) DEFAULT NULL COMMENT 'Nombre del vendedor para comisión de armazón',
  PRIMARY KEY (`id_venta`),
  KEY `fk_ventas_pacientes` (`id_paciente`),
  KEY `fk_ventas_consultas` (`consulta_id`),
  CONSTRAINT `fk_ventas_consultas` FOREIGN KEY (`consulta_id`) REFERENCES `consultas` (`id`) ON DELETE SET NULL,
